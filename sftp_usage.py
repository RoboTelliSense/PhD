...
for i in dir_in_list:
    ...
    paramiko.util.log_to_file('/tmp/ecelinsrvlog')
    dst_host = "ecelinsrv2.ece.gatech.edu"
    dst_port = 22
    dst_transport = paramiko.Transport((dst_host, dst_port))
    dst_password = "kotrisindh1@"
    dst_username = "gtg629v"
    dst_transport.connect(username = dst_username, password = dst_password)
    dst_sftp = paramiko.SFTPClient.from_transport (dst_transport)
    print("connected to remote destination, initiating transfer ...")

    #transfer: local -> remote destination
    dst_sftp.put(cfn_tmp, cfn_out)
    dst_sftp.close()
    dst_transport.close()
    print("file transferred to remote machine")
