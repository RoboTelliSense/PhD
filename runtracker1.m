...
%  param = estwarp_grad(frame, tmpl, param, opt);
  param = estwarp_condens1(frame, tmpl, param, opt);
  % do update
...
%    wimgs = wimgs(:,2:end);
  ...
  % draw result
  if (exist('GT','var'))
    trackpts(:,:,f) = param.tst_bestaffine_1x6([3,4,1;5,6,2])*[FP_initial_gt; ones(1,FP_num)];
    pts = cat(3, FP_initial_gt+repmat(sz'/2,[1,FP_num]), GT(:,:,f), trackpts(:,:,f));
    idx = find(pts(1,:,2) > 0);
    if (length(idx) > 0)
