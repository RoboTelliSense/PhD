...
if (isfield(opt,'affsig'))
  opt.param0 = affparam2geom(param.tst_bestAffineParams);
%disp(cat(1,p0,opt.param0,affparam2mat(opt.param0)));
disp(opt.param0);
end
param.tst_bestAffineParams = fminsearch(@esterrfunc, param.tst_bestAffineParams, opt.minopt, frm, tmpl, opt);
%p = lsqnonlin(@esterrfunc, p0, [],[], opt.minopt, frm, tmpl, opt);
param.wimg = warpimg(frm, param.tst_bestAffineParams, sz);
[err,afferr,param.recon,param.err] = esterrfunc(param.tst_bestAffineParams, frm, tmpl, opt);
