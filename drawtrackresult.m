...
%       .basis(tN,nb) : basis
%   param.tgt_best_affine2_1x6 : current estimate
...
if (drawopt.showcondens && isfield(param,'param') && isfield(param,'conf'))
  p = UTIL_2D_affine_tllptxty_to_abcdtxty(param.param(:,find(param.conf > drawopt.thcondens)));
  for i = 1:size(p,2)
...
UTIL_drawQuadFrom6affine_1x6(sz, param.tgt_best_affine2_1x6, 'Color','r', 'LineWidth',2.5);
