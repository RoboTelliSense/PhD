...
%===================================
We consider a simple training set, $S_{trg}=\{1,2,3,4,5,6,7\}$.  Table~\ref{table:Exp1_encoder} gives the encoder codebooks for this training set while Table~\ref{table:Exp1_decoder} gives the decoder codebooks for $m=2,3,4,5,6,7,8$.  As $m$ increases, less stages $Q$ are required to achieve the same distortion.  As a result, the stages drop from 3 to 1 in going from $m=1$ to $m=7$.  Also notice that encoder and decoder codebooks are the same except for the 2x3 case.  Using the encoder codebook, the reconstruction rms error at the output of the first stage is 0.6547.  A slightly higher rms error of 0.7557 is achieved using the decoder codebook.   Table~\ref{table:Exp2_detailed_computations} gives detailed computations for code-vectors picked at every stage, reconstructions, errors, etc for the 2x3 encoder codebook.  
