...
As a result, the stages drop from 3 to 1 in going from $m=1$ to $m=7$.  Figure~\ref{fig:RVQ_entanglement_tree} graphically displays construction of equivalent code-vectors for the 3x2 and 2x4 cases.  In the former case, the trained RVQ $\sigma$-tree is unentangled and the equivalent Voronoi cells are simply connected regions.  In the latter case, the RVQ $\sigma$-tree is mildly entangled.  As a result, the equivalent code-vectors    


							\begin{figure}
							\centering
							\subfigure[Unentangled tree using the 3x2 encoder codebook.]{\includegraphics[width=0.75\textwidth]{thesis2/RVQ_trg_1_to_7_equivalentCVs.pdf}}
							\subfigure[Entanglement using the 2x4 encoder codebook.  The paths shown in bold will not be taken by any input data-point.]{\includegraphics[width=0.75\textwidth]{thesis2/RVQ_trg_1_to_7_equivalentCVs_2.pdf}}
							...
							\caption{Training set $S_{trg}=\{1,2,3,4,5,6,7\}$.  Unentangled tree using the 3x2 encoder codebook.  Construction of equivalent code-vectors using encoder codebooks given in Table~\ref{table:Exp1_encoder}.  In both cases shown above, the encoder codebook is exactly the same as the decoder codebook.  In the 3x2 case, the tree is unentangled since the equivalent partitions in $\mathbb{R}$ are connected regions~\cite{1992_JNL_RVQ_Barnes}.  In the 2x4 case, there is mild entanglement.}
							\label{fig:RVQ_entanglement_tree}
							\end{figure}
