...
function [U_DxNt, S_Ntx1, muAB_Dx1, n] = sklm2(B_DxN2, U_DxN1, S_N1x1, muA_Dx1, N2, ff, K)
...
%-----------------------------------------------
    ...
    %part 2. user has input quite a bit    
        ...
        if (nargin >= 4 & isempty(muA_Dx1) == false)
            ...
            B_hat_DxN2      =   [B_zc_DxN2, weight*(muA_Dx1(:)-muB_Dx1)];
            muAB_Dx1        =   reshape((ff*N2*muA_Dx1(:) + n*muB_Dx1)/(n+ff*N2), size(muA_Dx1));
            n               =   n+ff*N2;
        end
        S_Ntx1              =   diag(S_N1x1);
        %>[Q,R,E]           =   qr([ ff*U_DxN1*S_Ntx1, B_hat_DxN2 ], 0); %> old way

		%step 3
        BprojU_N1xN2p1      =   U_DxN1'*B_hat_DxN2;                     %> projections on U
        data_res            =   B_hat_DxN2 - U_DxN1*BprojU_N1xN2p1;
        [q, dummy]          =   qr(data_res, 0);
        Q                   =   [U_DxN1 q];
        R                   =   [ff*diag(S_N1x1) BprojU_N1xN2p1; zeros([size(B_hat_DxN2,2) length(S_N1x1)]) q'*data_res];
