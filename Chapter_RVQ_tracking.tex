...
For PCA, it is assumed that an image $\mathbf{X}$ in $R^D$ is probabilistically generated from a subspace $\mathbf{U}$ spanned by earlier observed images.  The covariance matrix $\Sigma$ of the input training images can be written as follows,  
...
As mentioned earlier, VQ does not define a proper density in the observation space.  However, it is common to assign a probability measure to a new data point in proportion to the distance of the closest centroid~\cite{1999_JNL_Gaussian_roweis}

\begin{equation}
p(\mathbf{x}_i|\Phi) = \frac{e^{-\big(\dr\big)}} {\sum\limits_{i=1}^N e^{-\big(\dr\big)}}
\end{equation}

Here, $\boldsymbol\mu_k$ is closest centroid to $\mathbf{x}_i$, $Q_i$ is the number of stages required to decode $\mathbf{x}_i$, and $\lambda$ is a regularization parameter.  We use 4 different RVQ methods to compute which $\boldsymbol \mu_k$ $\mathbf{x}_i$ maps to,
...
\subsection{Converting $(a, b, c, d)$ to $(\theta, \lambda_1, \lambda_2, \phi)$}
%-------------------------------------------------
...
\subsection{Converting  $(\theta, \lambda_1, \lambda_2, \phi)$ to $(a, b, c, d)$}
%-------------------------------------------------
...
\end{equation}
								...
								\centering
								\fbox{
								\includegraphics[width=0.75\textwidth]{thesis/GRAPHICS_2D_left_turn.pdf}
								}
...
The code for this step is given in Listing~\ref{lst:UTIL_2D_affine_tllpxy_to_abcdxy}.
								...
								\subfigure[Desired output.]{\includegraphics[width=0.25\textwidth]{thesis/dataset_Dudek_desired_00001.pdf}}
...
%----------------------------------
\subsection{Forward affine transform}
%----------------------------------
...
%----------------------------------
\subsection{Inverse affine transform}
%----------------------------------
...
We conclude that affine warping is an effective way of extracting an object of interest and associated feature points from a given image.  The affine representation is rich enough to bound many objects of interest while minimizing inclusion of background pixels.  Therefore, this representation method is used in this work.
