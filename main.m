...
%-----------------------------------------5
    ...
    for f = PARAM.trg_B+1 : PARAM.ds_4_F
		...
		if (PARAM.in_bUseIPCA) trkIPCA = TRK_condensation(f, I_0t1, GT, RAND, PARAM, aMEAN, trkIPCA); end %estwarp_grad    (I_0t1, aIPCA, trkIPCA, PARAM);
		if (PARAM.in_bUseBPCA) trkBPCA = TRK_condensation(f, I_0t1, GT, RAND, PARAM, aMEAN, trkBPCA); end
		if (PARAM.in_bUseRVQ)  trkRVQ  = TRK_condensation(f, I_0t1, GT, RAND, PARAM, aMEAN, trkRVQ); end
		if (PARAM.in_bUseTSVQ) trkTSVQ = TRK_condensation(f, I_0t1, GT, RAND, PARAM, aMEAN, trkTSVQ); end
