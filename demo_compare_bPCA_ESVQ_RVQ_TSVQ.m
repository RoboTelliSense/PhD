%% This file compares PCA, aRVQE and aTSVQ by generating its own simple data.
% 
% It can be used without any outside data, except if dataset=2 is used.
%
% sw=1, sh=1 means scalar example
% 
% aRVQE.trg_1_featr_PxN: training XDRs from using gen.exe -l or my Matlab software
%
...
% usage example
% -------------
% A good way to check aRVQE is to use the 1:256 scalar example in the IDDM paper.  For that use the following settings:
% dataset     =   25
% aRVQE.in_3__maxQ   =   8
% aRVQE.in_4__M___      =   2
% Then aRVQE.mdl_3_CB_DxMP (the red channel of the codebooks) is 
%       m=1   m=2
%       ---   ---
% p=1  192.5  64.5
% p=2  -32    32
...
% The first M=2 numbers in aRVQE.mdl_3_CB_DxMP are the scalar codevectors for stage 1,
% the second M=2 numbers are the scalar codevectors for stage 2, and so on.
% 
% If test point is 192, feature vectors_PxN(:,192) produced by aRVQE.rule_stop_decoding='monRMSE' will give 
% [1;9;9;9;  9;9;9;9], the 9's, i.e., P+1s, showing early termination.
% So, the reproduction value is 192 and the error is 0.5.  
% feature vectors_PxN produced by gen.exe -l or aRVQE.rule_stop_decoding='maxQ' always gives full path,
...
% in order these are PCA, aRVQE (monRMSE), aRVQE (RoE), aTSVQ
% aTSVQ values can change though since the K-means can produce different
% results each time
%
% RVQ__old uses the aRVQE testing algorithm from first draft version of my thesis
...
%data input (5 different datasets, pick one of them)
    ...
    elseif (dataset==4) [DM2,sw,sh] ...                                     
                            =   DATAMATRIX_create_random_DM2;       %aRVQE error is large because apparently codebooks are clamped to 255            
    %random data  (complex)
...
%algorithm parameters
    ...
    %rvq    
    aRVQE.in_1__name          =   'aRVQE';
	aRVQE.in_2__mode          =   'tst';
    aRVQE.in_3__maxQ          =   8;                                          %number of stages  
    aRVQE.in_4__M___          =   2;                                          %number of codevectors/stage
    aRVQE.in_5__tSNR          =   1000;
    aRVQE.in_6__sw__          =   sw;                                         %snippet width
    aRVQE.in_7__sh__          =   sh;                                         %snippet height
    aRVQE.in_8__odir          =   '';
    aRVQE.in_9__trgR       =   'maxQ';                                     %can't have RoE because RoE only happens after training!
    aRVQE.in_10_tstR      =   'monRMSE';
...
%training    
    ...
    aRVQE                    =    RVQ__1_train     (DM2, aRVQE);           
    aTSVQ                   =    TSVQ_1_train     (DM2, aTSVQ); 
    
%testing
   
    aBPCA                   =   PCA__2_test(DM2, aBPCA);                  
    aRVQE                    =   RVQ__2_test(DM2, aRVQE);
...
%view
    ...
    numDisplayCols          =   10;
                                ...
                                figure;DATAMATRIX_display_DM2_as_image(aRVQE.mdl_3_CB_DxMP, sh, sw, aRVQE.mdl_1_Q__1x1, aRVQE.in_4__M___);title('aRVQE codebooks');
    ...
    [aBPCA.trg_5_rmse__1x1     aRVQE.trg_5_rmse__1x1    aTSVQ.trg_5_rmse__1x1         ]
    [aBPCA.tst_5_rmse__1x1     aRVQE.tst_5_rmse__1x1    aTSVQ.tst_5_rmse__1x1         ]
